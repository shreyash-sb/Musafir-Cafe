...
import { Compass, Eye, EyeOff, Lock, Mail, Sparkles, ArrowRight } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    notify("Welcome back to Musafir Cafe! You are logged in.");
  };
      ...
      <div className="w-full max-w-md rounded-[2.5rem] border border-primary/10 bg-white p-8 md:p-10 shadow-2xl shadow-primary/10 dark:border-white/10 dark:bg-[#1E110A]">
        <div className="text-center mb-8">
          ...
          <h1 className="mt-1 font-heading text-3xl font-bold text-primary dark:text-white">
            Welcome Back, Musafir
          </h1>
          <p className="mt-2 text-xs text-cafe-muted dark:text-white/70">
            Log in to manage your favorites, view past orders, and access wanderer club benefits.
          </p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <div>
            <div className="flex justify-between items-center mb-1">
              <label className="text-xs font-bold text-primary dark:text-white">Password</label>
